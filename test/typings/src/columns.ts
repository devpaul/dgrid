...
import "dgrid/test/data/base";
	...
	function runClassNameTests(){
		...
		assert.isTrue(query(".dgrid-content .dgrid-cell.field-description", domNode).every(function(cell){
				return (/desc-\w+ desc-row/).test((<Element> cell).className);
			}),
	...
	test.suite("columnSets", function(){
		...
		test.test("className property", function(){
			interface GridConstructor {
				new (args: any): dgrid.Grid & dgrid.ColumnSet;
			}
			const MyGrid: GridConstructor = <GridConstructor> declare([Grid, ColumnSet], {});
